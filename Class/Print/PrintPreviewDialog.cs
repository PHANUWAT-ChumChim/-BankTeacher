...
namespace BankTeacher.Class.Print
{
    ...
    class PrintPreviewDialog
    {
        ...
        public static void PrintLoan(System.Drawing.Printing.PrintPageEventArgs e, String SQLCode, String Day, String Month, String Year, String TeacherNo, String LoanNo,int Rowscount)
        {
            if (TeacherNo != "")
            {
                ...
                if (dt.Rows[0][6].ToString() != "0")
                {
                    ...
                    LimitMonthPay = BankTeacher.Bank.Menu.Month[PayNo-1].ToString() + " พ.ศ. " + (Yearpay + 543).ToString();
        ...
        // เเบบ ปริ้น รายงาน สำหรับ DataGridview
        // ตัวทดสอบ
        ...
        public static void PrintReportGrid(System.Drawing.Printing.PrintPageEventArgs e, DataGridView G,string header,string TextForm)
        {
            // ขนาดเส้นกำหนดของเเกนY ณ จุดที่จะตัด
            linesToPrint = e.PageBounds.Height-100;
            //================= Endline =====================
            // ปากกา//
                   ...
                   if (TextForm == "InfoLoan")
                    {
                        ...
                        e.Graphics.DrawRectangle(PenBlack, 50, TextY, x2 - 50, Size.Height-20);

                        Size = e.Graphics.MeasureString("infomeber", FonT(16, ThaiSarabun,FontStyle.Regular));
                        result = Centerset(e,infomeber, FonT(16,ThaiSarabun,FontStyle.Regular),BrushBlack,50,TextY,700,300,x2,Size.Height);
                        TextY += (Size.Height*result);

                        float start = TextY,end = 0;
                        for(int Grt = 0;Grt < Bank.Loan.InfoLoan.how_many_laps; Grt++)
                        {
                            string infoGuarantor = $"ชื่อ-นามสกุล : {Bank.Loan.InfoLoan.info_GuarantrN[Grt]}            ยอดค้ำ : {Bank.Loan.InfoLoan.info_Guarantrtotel[Grt]}\r\n" +
                            ...
                            result = Centerset(e,infoGuarantor, FonT(16, ThaiSarabun, FontStyle.Regular), BrushBlack, 50, TextY, 700, 300, x2, Size1.Height);
                            TextY += (Size1.Height * result);
                            end += Size.Height;
                        }
                        e.Graphics.DrawRectangle(PenBlack, 50, start, x2 - 50, end);
                        ...
                        e.Graphics.DrawString("เริ่ม",FonT(16, ThaiSarabun,FontStyle.Regular),BrushBlack,(x2 - Box) -Size1.Width,85);
                        e.Graphics.DrawString(Bank.Loan.InfoLoan.info_startdate, FonT(18,ThaiSarabun,FontStyle.Bold),BrushBlack,x2 - (Box/2)-Size.Width/2, 85);
                        e.Graphics.DrawRectangle(PenBlack,x2 - Box, 80, Box, 40);
                        ...
                        e.Graphics.DrawString("สิ้นสุด", FonT(16, ThaiSarabun,FontStyle.Regular), BrushBlack, (x2 - Box) - Size1.Width, 125);
                        e.Graphics.DrawString(Bank.Loan.InfoLoan.info_duedate, FonT(18, ThaiSarabun,FontStyle.Bold), BrushBlack, x2 - (Box / 2) - Size.Width / 2, 125);
                        e.Graphics.DrawRectangle(PenBlack, x2 - Box, 120, Box, 40);
                    }
                   else if(TextForm == "pay")
                   {
           
                   }
                    // ======================== จบข้อความในเเต่ละForm ========================
                    ...
                    for (int Columns = 0; Columns < G.ColumnCount; Columns++)
                    {
                        ...
                        (e, G.Columns[Columns].HeaderText, FonT(18, ThaiSarabun, FontStyle.Regular), setcut, 50, startTableY, out Rectangle_X, out Rectangle_Y, 0);
                ...
                for (int Rows = 0; Rows < G.RowCount; Rows++)
                {
                    if (Rows >= Currentposition && startTableY < linesToPrint)
                    {
                        ...
                        for (int Cells = 0; Cells < G.Rows[Rows].Cells.Count; Cells++)
                        {
                            ...
                            Class.Print.SetPrintMedtods.CutingCharAndString
                            (e, G.Rows[Rows].Cells[Cells].Value.ToString(), FonT(18, ThaiSarabun, FontStyle.Regular), setcut, 50, startTableY, out Rectangle_X, out Rectangle_Y, 0);
        ...
        //  เเบบ ปริ้น รายงาน สำหรับ SQL
        public static void Print(System.Drawing.Printing.PrintPageEventArgs e,DataSet Ds,string header)
        {
